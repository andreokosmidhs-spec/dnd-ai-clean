...
def _build_system_prompt(campaign: dict, character: dict, cards: List[dict], clock_hour: int, deck: Optional[List[dict]] = None, chaos: int = 0, recent_feedback: Optional[List[dict]] = None, dm_lessons: Optional[List[dict]] = None, canon_scenes: Optional[List[dict]] = None, current_location: Optional[Dict] = None) -> str:
    ...
    return (
        ...
        "DO NOT write 'they mention a name and a place' — write '\"…Evelyn. The old docks. "
        "Before second bell.\"' Even partial / muffled overhearing must be rendered as "
        "fragments in quotes ('\"…the locket…\" — the rest is lost to the wind'). When the "
        "player TALKS to an NPC, the NPC's reply must be a real spoken line in quotes, "
        "in that NPC's voice from their identity sheet. Summarized speech ('she explains "
        "that…', 'he tells you that…', 'they discuss the plan') is BANNED — write the "
        "spoken line. This rule overrides terseness; spending 2-3 quoted lines on "
        "important NPC speech is correct, not bloat.\n"
        "4d) TIME-OF-DAY IS A HARD CONSTRAINT. The TIME OF DAY block above defines what's "
        "PLAUSIBLE and what's FORBIDDEN at the current hour. Every sentence you write "
        "must be checkable against it. If the player's last action implies daytime "
        "bustle but the clock says midnight, you OVERRIDE the player's implicit "
        "expectation and reflect the actual hour — the streets are empty, the merchant "
        "is asleep, only the Watch is awake. Do not write 'dawn light', 'fresh bread', "
        "'merchants setting up', 'children playing' at night/midnight. Do not write "
        "'lanterns being lit' at midday. If you catch yourself drifting toward a "
        "wrong-time-of-day image, rewrite the sentence in the current period before "
        "committing it.\n"
