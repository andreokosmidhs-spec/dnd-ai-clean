    ...
    >
      <div className="space-y-6 text-slate-100">
        ...
        <section className="rounded-lg border border-slate-800 bg-slate-900/80 p-4">
          ...
          <p className="text-sm text-slate-200">
            Alignment: {characterData?.identity?.alignment || "—"}
          </p>
          <p className="text-sm text-slate-200">
            Sex: {characterData?.identity?.sex || "—"}
          </p>
        </section>
        ...
        <section className="rounded-lg border border-slate-800 bg-slate-900/80 p-4">
          ...
          <p className="text-sm text-slate-200">
            Race: {raceInfo?.base?.name || "—"}
          </p>
          {raceInfo?.subrace && (
            <p className="text-sm text-slate-200">
              Subrace: {raceInfo.subrace.name}
            </p>
          )}
          <p className="text-sm text-slate-200">
            ASI:{" "}
        ...
        <section className="rounded-lg border border-slate-800 bg-slate-900/80 p-4">
          ...
          <p className="text-sm text-slate-200">
            Class: {characterData?.class?.key || "—"}
          </p>
          <p className="text-sm text-slate-200">
            Saving Throws:{" "}
            {classInfo?.savingThrows?.join(", ") || "—"}
          </p>
          <p className="text-sm text-slate-200">
            Proficiencies:{" "}
            {classInfo
        ...
        <section className="rounded-lg border border-slate-800 bg-slate-900/80 p-4">
          ...
          <p className="text-sm text-slate-200">
            Tools:{" "}
            {backgroundInfo?.toolProficiencies?.join(", ") || "—"}
          </p>
          <p className="text-sm text-slate-200">
            Languages:{" "}
            {backgroundInfo
              ? backgroundInfo.languages?.count
                ? `Choose ${backgroundInfo.languages.count} language${
                    backgroundInfo.languages.count > 1 ? "s" : ""
                  }`
                : "—"
              : "—"}
          </p>
          {backgroundInfo?.feature && (
            <p className="text-sm text-slate-200">
              Feature: {backgroundInfo.feature.name} —{" "}
              {backgroundInfo.feature.description}
            </p>
          )}
        </section>
        ...
        <section className="rounded-lg border border-slate-800 bg-slate-900/80 p-4">
          ...
          <p className="text-sm text-slate-200">
            Age Category: {appearance.ageCategory || "—"}
          </p>
          <p className="text-sm text-slate-200">
            Height:{" "}
            {appearance.heightCm ? `${appearance.heightCm} cm` : "—"}
          </p>
          <p className="text-sm text-slate-200">
        ...
        <div className="flex justify-between items-center pt-2">
          <button
            type="button"
            onClick={onBack}
            className="px-4 py-2 rounded-md border border-slate-700 text-sm font-medium text-slate-200 hover:bg-slate-800 transition"
          >
            Back
          </button>
          ...
          <button
            ...
            className="px-4 py-2 rounded-md bg-amber-500 text-sm font-semibold text-slate-900 hover:bg-amber-400 disabled:opacity-60 disabled:cursor-not-allowed transition"
          >
