...
import { Dices, TrendingUp, TrendingDown, Minus, AlertCircle } from 'lucide-react';

/**
 * CheckRollPanel - UI for displaying ability checks and rolling dice
 * Shows when DM requests a check, allows player to roll, and displays results
 */
const CheckRollPanel = ({ checkRequest, character, onRollComplete, onDismiss }) => {
  ...
  const getModifier = () => {
    const abilityScore = character?.stats?.[checkRequest.ability] || 10;
  ...
  const rollDice = () => {
    ...
    setTimeout(() => {
      ...
      const success = total >= checkRequest.dc;
      const margin = total - checkRequest.dc;
      
      // Determine outcome tier
      let outcome = 'marginal_success';
      if (finalRoll === 20 || margin >= 10) {
  ...
  const getOutcomeColor = () => {
    ...
    switch(rollResult.outcome) {
      case 'critical_success': return 'bg-green-500/20 border-green-500';
      case 'clear_success': return 'bg-green-600/20 border-green-600';
      case 'marginal_success': return 'bg-blue-500/20 border-blue-500';
      case 'marginal_failure': return 'bg-yellow-500/20 border-yellow-500';
      case 'clear_failure': return 'bg-orange-500/20 border-orange-500';
      case 'critical_failure': return 'bg-red-500/20 border-red-500';
      default: return '';
    }
  };
  ...
  return (
    <div className="fixed bottom-4 right-4 w-96 bg-gray-900 border-2 border-amber-500 rounded-lg shadow-2xl z-50">
      ...
      <div className="p-4 space-y-3">
        ...
        {/* Roll Result */}
        {rollResult && (
          <div className={`p-4 rounded border-2 ${getOutcomeColor()}`}>
            {/* Dice Rolls */}
            ...
            <div className="text-center mt-3 pt-3 border-t border-gray-700">
              ...
              <div className="text-xs text-gray-400 mt-1 capitalize">
                {rollResult.outcome.replace('_', ' ')}
                {rollResult.margin !== 0 && (
                  <span> ({rollResult.margin > 0 ? '+' : ''}{rollResult.margin})</span>
                )}
              </div>
            </div>
