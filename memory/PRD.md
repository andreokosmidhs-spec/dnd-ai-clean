...
#### Off-Hook Spawner — Main Action Input + Clarification Gate (Feb 14, 2026)
- **Problem solved**: previously the off-hook spawner only fired from inside the Active Investigation's "What do you do?" panel. Typing an off-hook action in the MAIN action input (e.g. "I look for a sloop") went straight to lean_dm and the DM never spawned a new storyline.
- **Backend**: `/spawn-from-action` now has a **clarification gate** — when the player's action is < 6 words AND uses a vague verb (look/find/check/see/go/search/explore/wander/scout/watch/head, with filler stripping for "I/I'm/I'll" and -ing normalization), the endpoint returns `needs_clarification: true` + a Mercer-style clarification question themed by the tentative mission type, instead of spawning. The frontend captures the reply and re-calls the endpoint with `narration_context='Player clarification: …'` which bypasses the gate. Successful spawns now also return a `transition_narration` (1-2 sentence "DM plays along" bridge). Response shape now carries `is_off_hook: true` on the spawned branch too for consistency.
- **Frontend**: `AdventureLogWithDM.sendPlayerMessage` runs the spawn probe BEFORE `sendToAPI` for any action-type input ≥4 chars that isn't a roll/check confirmation. On `needs_clarification` → injects a `❓ DM asks…` beat and stores `pendingClarification`. On `spawned=true` → injects up to 3 beats (paused-summary, `🧭 New Direction`, `🪶 Scene Opens`) + sets activeStoryline + skips lean_dm. New source labels added.
- **Tested** via `testing_agent_v3_fork` (iteration_20.json): 20/22 new pytest pass + 23 regression pass; 2 LLM-bound tests were SKIPPED due to a `rstrip('e')` bug that turned 'see' → 's'. Bug fixed in same iteration. `is_off_hook=true` now echoed on the spawn branch per agent feedback. Pytest at `/app/backend/tests/test_spawn_clarification.py`.
